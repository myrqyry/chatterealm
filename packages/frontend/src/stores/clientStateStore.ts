...
import type { UnifiedSettings } from 'shared';
...
interface ClientState {
  ...
  // Game actions (these will be moved to TanStack Query)
  joinGame: (playerData: Partial<any>) => void;
  ...
  // Utility functions
  getPlayerById: (id: string) => any | undefined;
  getNearbyPlayers: (radius?: number) => any[];
  ...
  updatePerformanceMetrics: (metrics: Partial<ClientState['performanceMetrics']>) => void;
  clearCache: () => void;
}
...
export const useClientStateStore = create<ClientState>()(
  devtools(
    persist(
      (set, get) => {
        ...
        return {
          ...
          handleRegenerateWorld: () => {
            // This will be handled by TanStack Query
          },
          ...
          handleCreateCharacter: async (characterData) => {
            try {
              // This will be handled by TanStack Query
              set({
                gameMessage: `Character created! Welcome!`,
              });
            } catch (error) {
              set({
          ...
          getGameStats: () => {
            // This will be handled by TanStack Query
            return { totalPlayers: 0, activePlayers: 0, worldAge: 0 };
          },
